...
function progressBar(value, limit, { warn = 70, crit = 90 } = {}) {
  const p = pct(value, limit)
  const cls = p >= crit ? 'bar-crit' : p >= warn ? 'bar-warn' : 'bar-ok'
  return `
    <div class="progress-wrap">
      <div class="progress-bar">
        <div class="progress-fill ${cls}" style="width:${p}%"></div>
      </div>
      <span class="progress-label">${value.toLocaleString()} / ${limit.toLocaleString()}</span>
    </div>`
}
...
function subscriberRows(subscribers, limits) {
  ...
  const rows = subscribers.map(u => {
    const statusBadge = u.blocklisted
      ? '<span class="badge revoked">Blocked</span>'
      : '<span class="badge active">Active</span>'
    const blockBtn = u.blocklisted
      ? `<button class="btn-unblock" onclick="setBlock('${u.ptHash}', false)">Unblock</button>`
      : `<button class="btn-revoke" onclick="setBlock('${u.ptHash}', true)">Block</button>`
...
export function adminHTML(tokens, settings, subscribers, limits, adminToken) {
  ...
  const tokenRows = tokens.map(t => {
    ...
    const action   = isActive
      ? `<button class="btn-revoke" onclick="revokeToken('${t.hash}')">Revoke</button>`
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card.sub-card .value { color: #38bdf8; }
    .card.blocked-card .value { color: #fb923c; }
    ...
    .table-wrap { background: #1e293b; border: 1px solid #334155; border-radius: 10px; overflow: hidden; margin-bottom: 32px; }
...
<body>
  ...
  <main>
    <div class="stats">
      <div class="card sub-card">
        ...
        <div class="value">${subscribers.filter(s => !s.blocklisted).length}</div>
      </div>
  ...
  <script>
    ...
    async function setBlock(ptHash, block) {
      const label = block ? 'block' : 'unblock'
      if (!confirm((block ? 'Block' : 'Unblock') + ' this subscriber? ' + (block ? 'They will immediately lose access.' : 'They will regain access.'))) return
      const res = await fetch('/admin/blocklist?token=' + ADMIN_TOKEN, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ptHash, action: label })
      })
      if (res.ok) {
        showToast('Subscriber ' + label + 'ed.')
        setTimeout(() => location.reload(), 1000)
      } else {
